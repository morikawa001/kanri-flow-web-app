...
### 2.7 報告案docxの「変更／軽微変更」チェック規則
`common-docx.js` の `reportDocxDataForRow`（02_kanri-flow_報告案_共通.docx差し込み）では、報告区分に応じてチェック欄を以下で埋める：

**基本ルール（apply/publish共通）**
- **報告区分 = 変更** → 1つ目の変更＝■、2つ目の変更＝■、軽微＝□
- **報告区分 = 軽微変更** → 1つ目の変更＝■、2つ目の変更＝□、軽微＝■
- コード上の対応：`公表区分_上位変更`＝変更または軽微変更があれば■、`公表区分_下位変更`＝変更のみ■、`公表区分_軽微`＝軽微変更行があるときのみ■。

**publish：初回公表セット例外（2026-09-01追加）**
- publishでは依頼1の報告区分で「初回公表」を選ぶと、依頼2＝変更・依頼3＝軽微変更が自動追加される（`common-ui.js` の `renderRequestRows` 参照）。
- この3件セット（依頼1=初回公表＋依頼2=変更＋依頼3=軽微変更）は**1つの初回公表として扱い、セット内の変更・軽微変更は公表区分チェックや公表日・URL差し込みの対象にしない**。
- 判定はセット内（依頼1〜3）を除外し、**依頼4以降に明示された変更・軽微変更のみ**を対象とする：
  - 依頼4以降に「変更」 → 上位変更■・下位変更■・軽微□
  - 依頼4以降に「軽微変更」 → 上位変更■・下位変更□・軽微■
  - 依頼4以降に変更・軽微変更が無い → `公表区分_新規` のみ■（上位変更・下位変更・軽微は未チェック）
- `変更公表日`・`変更URL`・`軽微公表日`・`軽微URL` も**依頼4以降の該当行の値**を差し込む（セット内の日付・URLは使わない）。
- コード上の対応：`checkboxRows = linkedInitialSet ? relatedRows.slice(3) : relatedRows` から `cbChangeRow`／`cbMinorRow` を求めて判定する。`linkedInitialSet` は `pageMode === 'publish'` でガードしているためapplyページは本例外の影響を受けず、基本ルールのまま。

### 2.8 起案docxの「初回公表セット」扱い（2026-09-01追加）
`common-docx.js` の `docxDataForRow`（01_kanri-flow_起案_共通.docx差し込み）では、publishの初回公表セット（依頼1=初回公表＋依頼2=変更＋依頼3=軽微変更）の場合：
- **セット内の変更・軽微変更は起案docxに表示しない**（`報告事項`・`詳細内容`・`変更公表日`・`軽微変更公表日`・`変更URL`・`軽微変更URL`・`公表日`などから除外）。
- **依頼4以降**に変更・軽微変更がある場合のみ、その公表日・URLを表示する（報告案docxと同じ考え方）。
- 非セット時（例: 変更のみ・軽微変更のみ）やapplyページは従来どおり全行を表示。
- コード上の対応：`linkedInitialDraftSet = pageMode === 'publish' && targetRows[0].type === '初回公表' && targetRows[1].type === '変更' && targetRows[2].type === '軽微変更'` のとき `targetRows.slice(3)` から `cbChangeRow`／`cbMinorRow` を求めて表示に使う（applyはガードのため従来どおり）。
